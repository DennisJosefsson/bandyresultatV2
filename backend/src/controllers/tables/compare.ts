...
compareRouter.post('/compare', (async (
  ...
  const seasonNames = await Season.findAll({
    where: {
      seasonId: { [Op.in]: [startSeason, endSeason] },
    },
  ...
  if (!getCatTables || getCatTables.length === 0) {
    throw new NotFoundError({
      code: 404,
      message:
        'Lagen har inga inbördes matcher inlagda i databasen.',
      logging: false,
  ...
  const tables =
    compareCategoryTeamTables.parse(getCatTables)
  ...
  const allData = compareAllTeamTables.parse(
    getCompareAllGames
  )
...
where ranked_first_games = 1 or ranked_last_games < 11
order by "date" asc;
 `,
    {
      bind: { team_array: teamArray },
  ...
  const parsedFirstLastGames = parseFirstLast.parse(
    firstAndLatestGames
  )
